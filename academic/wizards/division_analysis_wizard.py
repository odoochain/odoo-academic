...
class AcademicDivisionAnalysisWizard(models.TransientModel):
    ...
    group_ids = fields.Many2many(
        'academic.group',
        'division_analysis_group_rel',
        string='Groups',
    )
    period_ids = fields.Many2many(
        'academic.period',
        'division_analysis_periods_rel',
        string='Period')
    company_id = fields.Many2one(
        'res.company',
        string='Company')
    division_analysis_id = fields.Many2one(
        'academic.division_analysis',
        string='Tablero',
        required=True,
        default=_default_division_analysis,
    )
    consider_disabled_person = fields.Boolean(
        string='Considerar Personas con Discapacidad?',
    )
    groups = fields.Char()
    periods = fields.Char()
    company = fields.Char()
    include_diagnosis_eval = fields.Boolean(
    ...
    def action_confirm(self):
        ...
        context = self.with_context(
            groups=groups, periods=periods, company=company,
            period_ids=period_ids, group_ids=group_ids, company_id=company_id,
            consider_disabled_person=consider_disabled_person,
            include_diagnosis_eval=include_diagnosis_eval,).env.context
